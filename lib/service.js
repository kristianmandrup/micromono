...
var Service = module.exports = function MicroMonoService() {
  ...
  if (this.route || this.asset || this.middleware) {
    console.log('Create router', this);
    var router = new Router(this);
    this.router = router;
    console.log('app', router.routeApp);
  ...
  if (this.use) {
    this.announcement.use = this.use;
  }
...
Service.prototype = {
  ...
  server: function(app) {
    ...
    if (router) {
      if (router.assetApp) {
        console.log('assetApp', router.assetApp);
        app.use(router.assetApp);
      }

      if (router.routeApp) {
        console.log('use routeApp', this.baseUrl, router.routeApp);
        // app.use(this.baseUrl, router.routeApp);
      }

      if (router.middlewareApp) {
        console.log('use middlewareApp', router.middlewareApp);
  ...
  startWebServer: function(host) {
    ...
    if (typeof host === 'function') {
      // host is a server instance, the main app
      this.mainApp = host;
      this.server(host);
